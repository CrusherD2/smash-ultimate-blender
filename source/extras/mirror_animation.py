...
def mirror_action_smash_y(
    ...
    # IK controls carry no Smash track, so anim_flip skips them, but they
    # still have to move to the other side with the body unless unchecked.
    custom_excluded = {name for name in excluded_bones if not is_ik_control(name)}
    ssp = getattr(context.scene, 'sub_scene_properties', None)
    if ssp is not None:
    ...
    from .mirror_custom_bones import (
        ...
        control_mirror_map, mirror_pole_angles,
    )
    ...
    try:
        for frame in frames:
            ...
            custom_pose = {name: matrix for name, matrix in custom_pose.items()
                           if name not in custom_excluded}
