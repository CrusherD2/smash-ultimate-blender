...
- An experimental Rust accelerator for generated two-bone chains remains bundled but
  **disabled by default**: it cannot reproduce Eigen's SVD arithmetic exactly on
  near-straight chains. `SUB_NATIVE_IK=1` verifies every native candidate against
  Blender (exact, but slower than the default); `SUB_NATIVE_IK=experimental` opts
  into unverified native results and must not be used when exact matching is required
